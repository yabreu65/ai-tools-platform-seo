...
const trackingSetupSchema = z.object({
  project_name: z.string().min(1, 'Nombre del proyecto es requerido'),
  website_url: z.string().url('URL del sitio web inválida'),
  keywords: z.array(z.string()).min(1, 'Al menos una keyword es requerida').max(100, 'Máximo 100 keywords por proyecto'),
  ...
  tracking_frequency: z.enum(['daily', 'weekly', 'monthly']).default('weekly'),
  alerts_enabled: z.boolean().default(true),
  alert_thresholds: z.object({
    position_change: z.number().min(1).max(50).default(5),
    visibility_change: z.number().min(1).max(100).default(20),
    new_competitor: z.boolean().default(true),
...
const trackingDataSchema = z.object({
  project_id: z.string().min(1, 'ID del proyecto es requerido'),
  date_range: z.enum(['7d', '30d', '90d', '6m', '1y']).default('30d'),
  keywords: z.array(z.string()).optional(),
  competitors: z.array(z.string()).optional(),
  metrics: z.array(z.enum(['position', 'visibility', 'traffic', 'features'])).default(['position', 'visibility'])
});
...
interface TrackingProject {
  ...
  alerts: {
    id: string;
    type: 'position_change' | 'visibility_change' | 'new_competitor' | 'serp_feature';
    keyword?: string;
    message: string;
...
const generateHistoricalRankingData = (keyword: string, days: number): RankingData[] => {
  ...
  if (trendType < 0.3) trendDirection = -0.1; // Mejorando (posición bajando)
  else if (trendType < 0.6) trendDirection = 0.1; // Empeorando
  // else estable
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    const previousPosition = i === days - 1 ? null : data[data.length - 1]?.position || null;
    const change = previousPosition ? position - previousPosition : 0;
    
    // SERP features (más probable en posiciones altas)
    const serpFeatures = [];
    if (position <= 3 && Math.random() > 0.7) serpFeatures.push('featured_snippet');
    if (position <= 5 && Math.random() > 0.8) serpFeatures.push('sitelinks');
    if (Math.random() > 0.9) serpFeatures.push('images');
...
const generateKeywordTracking = (keywords: string[], days: number): KeywordTracking[] => {
  return keywords.map(keyword => {
    ...
    // Calcular métricas
    const positions = historicalData.map(d => d.position).filter(p => p !== null) as number[];
    ...
    const competitorPositions: { [competitor: string]: number | null } = {
...
const generateCompetitorTracking = (competitors: string[], keywords: string[]): CompetitorTracking[] => {
...
const generateAlerts = (keywordTracking: KeywordTracking[]): TrackingProject['alerts'] => {
  const alerts: TrackingProject['alerts'] = [];
  const now = new Date();
...
router.post('/data', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const competitorTracking = generateCompetitorTracking(
      filterCompetitors && filterCompetitors.length > 0 ? filterCompetitors : mockCompetitors,
      keywordsToTrack
    );
    ...
    const filteredData = {
      keywords: keywordTracking.map(kw => {
        const filtered: any = { keyword: kw.keyword };
      ...
      summary: {
        ...
        avg_position: Math.round(
          keywordTracking.reduce((sum, kw) => sum + (kw.current_position || 100), 0) / keywordsToTrack.length
        ),
        visibility_score: Math.round(
          keywordTracking.reduce((sum, kw) => sum + kw.visibility_score, 0) / keywordsToTrack.length
        ),
    ...
    res.json({
      ...
      query_params: {
        ...
        keywords_filtered: filterKeywords?.length || 0,
        competitors_filtered: filterCompetitors?.length || 0
      }
    });
