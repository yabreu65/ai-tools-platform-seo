...
const trendsSchema = z.object({
  keywords: z.array(z.string()).min(1, 'Al menos una keyword es requerida').max(20, 'Máximo 20 keywords por análisis'),
  time_range: z.enum(['1m', '3m', '6m', '1y', '2y', '5y']).default('1y'),
  country: z.string().default('US'),
  language: z.string().default('en'),
  include_seasonality: z.boolean().default(true),
  include_predictions: z.boolean().default(true),
  include_related_trends: z.boolean().default(true),
...
interface TrendAnalysis {
  ...
  predictions: {
    next_3_months: PredictionPoint[];
    next_6_months: PredictionPoint[];
...
const generateHistoricalData = (keyword: string, timeRange: string, granularity: string): TrendDataPoint[] => {
  ...
  let trendSlope = 0;
  if (trendType < 0.3) trendSlope = 0.5; // Creciente
  else if (trendType < 0.6) trendSlope = -0.3; // Decreciente
  // else estable
  ...
  for (let i = dataPoints - 1; i >= 0; i--) {
    ...
    if (isCommercial) {
      ...
      if (month === 10) seasonalEffect = 20; // Noviembre
      else if (month === 11) seasonalEffect = 15; // Diciembre
      else if (month === 0) seasonalEffect = -10; // Enero (post-navidad)
      else if (month === 6 || month === 7) seasonalEffect = 10; // Verano
    }
    ...
    const value = Math.max(0, Math.min(100, 
      currentInterest + trendEffect + seasonalEffect + randomVariation
    ));
...
const analyzeSeasonality = (data: TrendDataPoint[], keyword: string) => {
...
const generatePredictions = (data: TrendDataPoint[], keyword: string) => {
  ...
  const next3Months: PredictionPoint[] = [];
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    if (trendSlope > 2) trendDirection = 'up';
    else if (trendSlope < -2) trendDirection = 'down';
  ...
  const next6Months: PredictionPoint[] = [...next3Months];
  ...
  for (let i = 4; i <= 6; i++) {
    ...
    let trendDirection: 'up' | 'down' | 'stable' = 'stable';
    if (trendSlope > 1) trendDirection = 'up';
    else if (trendSlope < -1) trendDirection = 'down';
  ...
  if (trendSlope > 3) trendForecast = 'Crecimiento fuerte esperado';
  else if (trendSlope > 1) trendForecast = 'Crecimiento moderado esperado';
  else if (trendSlope < -3) trendForecast = 'Declive fuerte esperado';
  else if (trendSlope < -1) trendForecast = 'Declive moderado esperado';
...
const generateRelatedTrends = (keyword: string): RelatedTrend[] => {
  const baseKeywords = [
...
const generateInsights = (analysis: Omit<TrendAnalysis, 'insights'>): TrendAnalysis['insights'] => {
...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    for (const keyword of keywords) {
      ...
      if (recentGrowth > 10) trendDirection = 'rising';
      else if (recentGrowth < -10) trendDirection = 'declining';
      else if (Math.abs(recentGrowth) > 20) trendDirection = 'volatile';
      ...
      // Crear análisis base
      const baseAnalysis: Omit<TrendAnalysis, 'insights'> = {
        keyword,
