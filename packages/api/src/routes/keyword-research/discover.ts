...
const discoverSchema = z.object({
  seed_keywords: z.array(z.string()).min(1, 'Al menos una keyword es requerida'),
  country: z.string().default('US'),
  language: z.string().default('en'),
  include_related: z.boolean().default(true),
  include_longtail: z.boolean().default(true),
  include_questions: z.boolean().default(true),
  min_volume: z.number().min(0).default(0),
  max_volume: z.number().optional(),
  min_difficulty: z.number().min(0).max(100).default(0),
  max_difficulty: z.number().min(0).max(100).default(100),
  search_intent: z.enum(['all', 'informational', 'navigational', 'commercial', 'transactional']).default('all'),
...
interface KeywordSuggestion {
  keyword: string;
  type: 'related' | 'longtail' | 'question';
  search_volume: number;
  keyword_difficulty: number;
  cpc: number;
  competition: number;
...
const generateKeywordData = (keyword: string, type: 'seed' | 'related' | 'longtail' | 'question' = 'seed'): KeywordData => {
...
const generateRelatedKeywords = (baseKeyword: string, count: number): string[] => {
...
const generateLongTailKeywords = (baseKeyword: string, count: number): KeywordSuggestion[] => {
...
const generateQuestionKeywords = (baseKeyword: string, count: number): KeywordSuggestion[] => {
...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    let filteredResults = results.filter(keyword => {
      // Filtro de volumen
      if (keyword.search_volume < min_volume) return false;
      if (max_volume && keyword.search_volume > max_volume) return false;
      
      // Filtro de dificultad
      if (keyword.keyword_difficulty < min_difficulty) return false;
      if (keyword.keyword_difficulty > max_difficulty) return false;
      
      // Filtro de intención de búsqueda
      if (search_intent !== 'all' && keyword.search_intent !== search_intent) return false;
