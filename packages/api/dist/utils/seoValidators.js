...
exports.generateUserAgent = exports.sanitizeUrl = exports.validatePlanLimits = exports.validateExportFormat = exports.validateRobotsConfig = exports.validateSitemapConfig = exports.validateUrl = void 0;
...
const validateSitemapConfig = (config) => {
    const errors = [];
    // Validar URL
    const urlValidation = (0, exports.validateUrl)(config.url);
...
exports.validateSitemapConfig = validateSitemapConfig;
const validateRobotsConfig = (config) => {
    const errors = [];
    // Validar URL del sitio
    const urlValidation = (0, exports.validateUrl)(config.siteUrl);
    if (!urlValidation.isValid) {
    ...
    if (config.sitemapUrl) {
        const sitemapValidation = (0, exports.validateUrl)(config.sitemapUrl);
...
exports.validateRobotsConfig = validateRobotsConfig;
// Validar formato de exportación
const validateExportFormat = (format, allowedFormats) => {
    const errors = [];
...
const validatePlanLimits = (userPlan, requestedDepth, planLimits) => {
    const errors = [];
    if (planLimits.maxDepth !== -1 && requestedDepth > planLimits.maxDepth) {
        errors.push(`Tu plan ${userPlan} permite máximo ${planLimits.maxDepth} niveles de profundidad. ` +
