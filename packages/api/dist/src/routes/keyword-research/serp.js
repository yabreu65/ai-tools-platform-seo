...
const serpSchema = zod_1.z.object({
    keyword: zod_1.z.string().min(1, 'Keyword es requerida'),
    location: zod_1.z.string().default('United States'),
    language: zod_1.z.string().default('en'),
    device: zod_1.z.enum(['desktop', 'mobile', 'tablet']).default('desktop'),
    include_paid_results: zod_1.z.boolean().default(false),
    include_local_results: zod_1.z.boolean().default(true),
    include_features_analysis: zod_1.z.boolean().default(true),
    depth: zod_1.z.enum(['basic', 'detailed', 'comprehensive']).default('detailed')
});
// Función para generar features SERP
const generateSerpFeatures = (keyword, device) => {
    const features = [];
...
const generateOrganicResults = (keyword, count = 10) => {
    const domains = [
...
const generatePaidResults = (keyword, includePaid) => {
    if (!includePaid || Math.random() > 0.7)
        return [];
    const adDomains = [
...
const generateLocalResults = (keyword, includeLocal) => {
    if (!includeLocal || !keyword.includes('near') && Math.random() > 0.3)
        return [];
    const businessTypes = [
...
const analyzeCompetitors = (organicResults) => {
    ...
    competitorMap.forEach((results, domain) => {
        ...
        const opportunities = [];
        if (avgMetrics.da > 70)
            strengths.push('Alta autoridad de dominio');
        else if (avgMetrics.da < 40)
            weaknesses.push('Baja autoridad de dominio');
        if (avgMetrics.backlinks > 10000)
            strengths.push('Fuerte perfil de backlinks');
        else if (avgMetrics.backlinks < 1000)
            weaknesses.push('Perfil de backlinks débil');
        if (avgMetrics.contentLength > 2000)
            strengths.push('Contenido extenso y detallado');
        else if (avgMetrics.contentLength < 1000)
            weaknesses.push('Contenido superficial');
        if (bestPosition > 5)
            opportunities.push('Oportunidad de superar en top 5');
...
const analyzeContent = (organicResults, keyword) => {
    const avgContentLength = Math.round(organicResults.reduce((sum, r) => sum + r.technical_metrics.content_length, 0) / organicResults.length);
...
router.post('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const { keyword, location, language, device, include_paid_results, include_local_results, include_features_analysis, depth } = validatedData;
