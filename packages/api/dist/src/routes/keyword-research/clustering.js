...
const clusteringSchema = zod_1.z.object({
    keywords: zod_1.z.array(zod_1.z.string()).min(5, 'Mínimo 5 keywords requeridas para clustering').max(500, 'Máximo 500 keywords por análisis'),
    method: zod_1.z.enum(['semantic', 'intent', 'metrics', 'hybrid']).default('hybrid'),
    min_cluster_size: zod_1.z.number().min(2).max(20).default(3),
    max_clusters: zod_1.z.number().min(2).max(50).default(15),
    similarity_threshold: zod_1.z.number().min(0.1).max(1.0).default(0.6),
    include_metrics: zod_1.z.boolean().default(true),
    include_intent_analysis: zod_1.z.boolean().default(true),
    language: zod_1.z.string().default('en')
});
// Función para determinar intención de búsqueda
const determineSearchIntent = (keyword) => {
...
const calculateSemanticSimilarity = (keyword1, keyword2) => {
...
const calculateIntentSimilarity = (intent1, intent2) => {
    var _a;
    if (intent1 === intent2)
        return 1.0;
    ...
    return ((_a = intentRelations[intent1]) === null || _a === void 0 ? void 0 : _a[intent2]) || 0.1;
};
// Función para calcular similitud de métricas
const calculateMetricsSimilarity = (metrics1, metrics2) => {
...
const performClustering = (keywordMetrics, method, minClusterSize, maxClusters, similarityThreshold) => {
    const clusters = [];
    const unclustered = [];
    ...
    keywordMetrics.forEach(centroidKeyword => {
        if (processed.has(centroidKeyword.keyword) || clusters.length >= maxClusters)
            return;
        ...
        keywordMetrics.forEach(keyword => {
            if (!processed.has(keyword.keyword)) {
                ...
                if (similarity >= similarityThreshold) {
                    clusterKeywords.push(Object.assign(Object.assign({}, keyword), { similarity_to_centroid: similarity, cluster_relevance_score: similarity * (keyword.search_volume / 10000) // Factor de volumen
                     }));
                }
            }
        });
        ...
        if (clusterKeywords.length >= minClusterSize) {
            ...
            const avgSearchVolume = Math.round(clusterKeywords.reduce((sum, kw) => sum + kw.search_volume, 0) / clusterKeywords.length);
            const avgDifficulty = Math.round(clusterKeywords.reduce((sum, kw) => sum + kw.difficulty, 0) / clusterKeywords.length);
            const avgCpc = Math.round((clusterKeywords.reduce((sum, kw) => sum + kw.cpc, 0) / clusterKeywords.length) * 100) / 100;
...
router.post('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const { keywords, method, min_cluster_size, max_clusters, similarity_threshold, include_metrics, include_intent_analysis, language } = validatedData;
        // Simular tiempo de procesamiento
        yield new Promise(resolve => setTimeout(resolve, 4000 + Math.random() * 6000));
        // Generar métricas para todas las keywords
        const keywordMetrics = generateKeywordMetrics(keywords);
        // Realizar clustering
        const { clusters, unclustered } = performClustering(keywordMetrics, method, min_cluster_size, max_clusters, similarity_threshold);
        ...
        if (clusters.length > 0) {
            ...
            recommendations.content_strategy.push(`Priorizar el cluster "${topCluster.name}" con ${topCluster.size} keywords`, `Crear contenido pillar para intención ${intentAnalysis.dominant_intent}`, `Aprovechar clusters con alto volumen de búsqueda promedio`);
        }
        // Recomendaciones de optimización de clusters
        if (clusteringScore < 70) {
            recommendations.cluster_optimization.push('Ajustar threshold de similitud para mejorar agrupación', 'Considerar método de clustering híbrido para mejores resultados');
        }
        if (unclusteredKeywords > totalKeywords * 0.3) {
            recommendations.cluster_optimization.push('Muchas keywords sin agrupar - revisar criterios de similitud', 'Considerar crear clusters adicionales para keywords aisladas');
        }
        // Recomendaciones de expansión de keywords
        recommendations.keyword_expansion.push('Buscar keywords long-tail para cada cluster identificado', 'Investigar keywords relacionadas semánticamente', 'Analizar keywords de competidores en cada cluster');
        const result = {
