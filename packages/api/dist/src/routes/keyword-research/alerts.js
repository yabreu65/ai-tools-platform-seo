...
const alertConfigurationSchema = zod_1.z.object({
    alert_name: zod_1.z.string().min(1, 'Nombre de la alerta es requerido').max(100, 'Nombre muy largo'),
    alert_type: zod_1.z.enum(['position_change', 'visibility_change', 'new_competitor', 'serp_feature', 'traffic_change', 'keyword_opportunity']),
    keywords: zod_1.z.array(zod_1.z.string()).min(1, 'Al menos una keyword es requerida').max(50, 'Máximo 50 keywords por alerta'),
    conditions: zod_1.z.object({
        position_threshold: zod_1.z.number().min(1).max(100).optional(),
        position_change_threshold: zod_1.z.number().min(1).max(50).optional(),
        visibility_threshold: zod_1.z.number().min(1).max(100).optional(),
        traffic_change_threshold: zod_1.z.number().min(1).max(500).optional(),
        serp_features: zod_1.z.array(zod_1.z.enum(['featured_snippet', 'sitelinks', 'images', 'videos', 'local_pack', 'knowledge_panel', 'people_also_ask'])).optional(),
        competitor_domains: zod_1.z.array(zod_1.z.string().url()).max(10, 'Máximo 10 competidores').optional(),
        frequency_check: zod_1.z.enum(['daily', 'weekly', 'monthly']).default('weekly')
    }),
    notification_settings: zod_1.z.object({
        ...
        webhook_enabled: zod_1.z.boolean().default(false),
        webhook_url: zod_1.z.string().url().optional(),
        slack_enabled: zod_1.z.boolean().default(false),
        slack_webhook: zod_1.z.string().url().optional(),
...
const alertQuerySchema = zod_1.z.object({
    alert_type: zod_1.z.enum(['position_change', 'visibility_change', 'new_competitor', 'serp_feature', 'traffic_change', 'keyword_opportunity']).optional(),
    status: zod_1.z.enum(['active', 'paused', 'triggered']).optional(),
...
const alertUpdateSchema = zod_1.z.object({
    alert_id: zod_1.z.string().min(1, 'ID de alerta es requerido'),
    ...
    conditions: zod_1.z.object({
        ...
        visibility_threshold: zod_1.z.number().min(1).max(100).optional(),
        traffic_change_threshold: zod_1.z.number().min(1).max(500).optional(),
        frequency_check: zod_1.z.enum(['daily', 'weekly', 'monthly']).optional()
    }).optional(),
    notification_settings: zod_1.z.object({
        email_enabled: zod_1.z.boolean().optional(),
        email_recipients: zod_1.z.array(zod_1.z.string().email()).max(10).optional(),
        priority: zod_1.z.enum(['low', 'medium', 'high']).optional()
    }).optional()
});
// Función para generar triggers de ejemplo
const generateAlertTriggers = (alertType, keywords, count = 3) => {
    const triggers = [];
    const now = new Date();
    for (let i = 0; i < Math.min(count, keywords.length); i++) {
        ...
        switch (alertType) {
            case 'position_change':
                ...
                previousValue = currentValue - positionChange;
            ...
            case 'visibility_change':
                ...
                previousValue = Math.max(0, Math.min(100, currentValue - visibilityChange));
            ...
            case 'new_competitor':
                ...
                severity = currentValue <= 5 ? 'high' : 'medium';
                conditionMet = {
                ...
                details.competitor_changes = [{
                        ...
                        position_change: -currentValue // Negativo porque es nuevo
                    }];
                break;
            case 'traffic_change':
                const trafficChange = Math.floor(Math.random() * 200) - 100; // -100% a +100%
                currentValue = Math.floor(Math.random() * 10000) + 1000;
                previousValue = Math.max(0, currentValue - currentValue * (trafficChange / 100));
...
const generatePerformanceMetrics = (createdDate) => {
...
router.post('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const { alert_name, alert_type, keywords, conditions, notification_settings, active, location, language, device } = validatedData;
        ...
        const configuredAlert = {
            ...
            alert_type,
            keywords,
            conditions: Object.assign(Object.assign({}, conditions), { frequency_check: conditions.frequency_check || 'weekly' }),
            notification_settings: {
                email_enabled: (_a = notification_settings.email_enabled) !== null && _a !== void 0 ? _a : true,
                email_recipients: notification_settings.email_recipients || [],
                webhook_enabled: (_b = notification_settings.webhook_enabled) !== null && _b !== void 0 ? _b : false,
                webhook_url: notification_settings.webhook_url,
                slack_enabled: (_c = notification_settings.slack_enabled) !== null && _c !== void 0 ? _c : false,
...
router.post('/list', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const { alert_type, status, priority, date_from, date_to, limit, offset } = validatedData;
        ...
        const statuses = ['active', 'paused', 'active', 'active']; // Más activas que pausadas
        ...
        res.json({
            ...
            data: {
                ...
                summary: {
                    ...
                    alerts_triggered_today: filteredAlerts.filter(a => a.last_triggered && new Date(a.last_triggered).toDateString() === new Date().toDateString()).length,
...
router.get('/triggers', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // Simular tiempo de consulta
        yield new Promise(resolve => setTimeout(resolve, 400 + Math.random() * 600));
        ...
        res.json({
            ...
            data: {
                ...
                summary: {
                    ...
                    triggers_today: filteredTriggers.filter(t => new Date(t.trigger_date).toDateString() === new Date().toDateString()).length
                }
            }
        });
    }
